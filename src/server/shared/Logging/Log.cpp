...
void Log::vlog(std::string const& filter, LogLevel level, char const* str, va_list argptr)
{
    ...
    write(filter, level, text, nullptr);
}

void Log::write(std::string const& type, LogLevel level, char const* str, char const* param) const
{
...
void Log::outCharDump(char const* str, uint32 accountId, uint32 guid, char const* name)
{
    ...
    write("entities.player.dump", LOG_LEVEL_INFO, ss.str().c_str(), param.str().c_str());
}
...
void Log::outCommand(uint32 account, const char * str, ...)
{
    ...
    write("commands.gm", LOG_LEVEL_INFO, text, ss.str().c_str());
}
