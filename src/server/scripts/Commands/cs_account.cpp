...
public:
    ...
    static bool HandleAccountBoostCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
            return false;
        ...
        SetBoosting(session, accountId, true);
        handler->SendSysMessage(LANG_COMMAND_BOOST_OK);
