    ...
    public:
        ...
        struct boss_vazrudenAI : public BossAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (ShieldSlam_Timer <= diff)
                {
                    if (Unit* victim = me->GetVictim())
                        DoCast(victim, SPELL_SHIELD_SLAM);

                    ShieldSlam_Timer = urand(10 * IN_MILLISECONDS, 15.5 * IN_MILLISECONDS);
                }
                else
                    ShieldSlam_Timer -= diff;
...
class boss_vazruden_the_herald : public CreatureScript
{
    public:
        ...
        struct boss_vazruden_the_heraldAI : public ScriptedAI
        {
            ...
            void DoAction(int32 actionId) override
            {
                if (Creature* Nazan = Unit::GetCreature(*me, NazanGUID))
                    Nazan->AI()->DoAction(1);
            }
                ...
                switch (phase)
                {
                    ...
                    default: // adds do the job now
                        if (check <= diff)
                        {
                            ...
                            {
                                me->SummonGameObject(DUNGEON_MODE(GO_FEL_IRON_CHEST_NORMAL, GO_FEL_IRON_CHECT_HEROIC), VazrudenMiddle[0], VazrudenMiddle[1], VazrudenMiddle[2], 0, { }, 0);
