...
class boss_mandokir : public CreatureScript
{
    public:
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_OHGAN_IS_DEATH:
                        ...
                        me->GetMap()->SetWorldState(WORLDSTATE_OHGANOT_SO_FAST, 0);
                        break;
        ...
        {
            return GetInstanceAI<boss_mandokirAI>(creature);
        }
};
...
class npc_ohgan : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetInstanceAI<npc_ohganAI>(creature);
        }
};
...
class npc_chained_spirit : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return GetInstanceAI<npc_chained_spiritAI>(creature);
        }
};
...
class DecapitatePredicate 
{
    public:
        ...
        {
            return object && object->ToUnit() && _caster->GetVictim() && _caster->GetVictim()->GetGUID() == object->ToUnit()->GetGUID();
        }
...
class spell_mandokir_decapitate : public SpellScript
{
    ...
    {
        targets.remove_if(DecapitatePredicate(GetCaster()));
